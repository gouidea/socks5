...
function pre_install(){
    ...
    read -p "(默认密码: gouidea):" gostpwd
	[ -z "$gostpwd" ] && gostpwd="gouidea"
	echo
	if ! [[ $gostpwd =~ ^[a-zA-Z0-9]+$ ]];then
	echo -e "\033[44;31m 密码里混进了奇怪的字符！密码改为默认密码gouidea。请在脚本之后完毕后用“gost set”命令重新设置密码！\033[0m"
	gostpwd="gouidea"
	fi
    echo "---------------------------"
...
function config_gost(){
    ...
    echo "{
    \"ServeNodes\": [
        \"socks://gouidea:${gostpwd}@:${gostport}\"
    ]
}
...
if [ -s $filename ];then
#取密码
gostpwd=$(cat $filename | grep 'gouidea' | cut -d':' -f3 | cut -d'@' -f1)
#取端口
gostport=$(cat $filename | grep 'gouidea' | cut -d':' -f4 | cut -d'"' -f1)
...
echo -e "\033[42;44mby gouidea @ 2021/1/4\033[0m"
...
\033[42;44mby gouidea @ 2021/1/4\033[0m
